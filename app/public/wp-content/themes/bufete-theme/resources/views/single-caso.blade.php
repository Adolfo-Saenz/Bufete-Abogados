...
@if ($client_id == $user_id || $lawyer_id == $user_id || $perfil[0] == 'administrator')
    {{-- Heading --}}
    ...
    <section class="px-10 py-10">
        ...
        <p class="text-[20px]">{!! nl2br(e($desc)) !!}</p>
    </section>
    ...
    <section class="px-10 py-10">
        <div class="flex flex-row max-sm:flex-col items-center items-justified-space-between">
            ...
            <a href="http://bufete-abogados.local/events-list?caso_id={{get_the_ID()}}">Ver más</a>
        </div>
        <div class="flex md:flex-row items-justified-space-between">
            ...
            <div class="grid sm:grid-cols-2 gap-10">
                ...
                @foreach ($eventos->posts as $evento)
                    ...
                    @endphp
                    @if($cuenta < 2)
                        @if($caso_relacionado->ID == get_the_ID())
                            @php
                                $cuenta++;
                            @endphp
